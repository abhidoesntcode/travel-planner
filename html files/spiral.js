...
class Star {
    constructor(cameraZ, cameraTravelDistance) {
        ...
        // Expanded native distance bounds of particles 5x
        this.distance = 150 * Math.random() + 50;
    ...
    render(p, controller) {
        ...
        if (q > 0) {
            ...
            // Returned sizes back down so they look like crisp dots
            const dotSize = 15 * this.strokeWeightFactor * sizeMultiplier;
...
class AnimationController {
    constructor(canvas, ctx, dpr, size) {
        ...
        this.viewZoom = 1800; // Skyrocketed zoom scale
        this.numberOfStars = 90000; // 3x Massive scale while keeping circular geometry
    ...
    spiralPath(p) {
        ...
        // Expanded core radius multiplier to fit full page
        const r = 800 * Math.sqrt(p);
    ...
    showProjectedDot(position, sizeFactor) {
        ...
        if (position.z > newCameraZ) {
            ...
            // Reduce base multiplier to keep dots from ballooning
            const sw = 30 * sizeFactor / dotDepthFromCamera;
            const radius = Math.max(0.5, sw / 2);

            // Reverted back to beautiful circular curves
            this.ctx.beginPath();
    ...
    render() {
        ...
        ctx.fillStyle = isDark ? 'rgba(255, 255, 255, 0.7)' : 'rgba(0, 0, 0, 0.7)'; // White for dark mode, subtle black for light mode
    ...
    drawTrail(t1) {
        for (let i = 0; i < this.trailLength; i++) {
            ...
            const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
            this.ctx.fillStyle = isDark ? 'rgba(255, 255, 255, 0.95)' : 'rgba(0, 0, 0, 0.9)';
            this.ctx.lineWidth = sw * 4; // Thick enough to see, but not massive
            ...
            // Reverted back to curved sweeps for the trail!
            this.ctx.beginPath();
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Place canvas in background with Tailwind classes
    canvas.className = "fixed inset-0 w-full h-full -z-10 pointer-events-none opacity-60";

    // Fallback inline styles if Tailwind misses anything
